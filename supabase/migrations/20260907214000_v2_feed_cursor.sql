...
begin
  ...
  with page as (
    select l.id,l.caption,l.taken_at,p.username,coalesce(d.name,l.drink_type::text) drink_name,a.thumbnail_key,
    ...
    where l.deleted_at is null and can_view_user(p_viewer,l.user_id)
      and (v_taken is null or (l.taken_at,l.id) < (v_taken,v_id))
    order by l.taken_at desc,l.id desc limit least(greatest(p_limit,1),20)
  ), rows as (
    select jsonb_build_object('id',id,'authorName',username,'drinkName',drink_name,'caption',caption,'takenAt',taken_at,'thumbnailKey',thumbnail_key,'cheersCount',cheers_count,'viewerCheered',viewer_cheered,'pending',false) row,taken_at,id from page
  )
