create table if not exists public.notifications_v2(id uuid primary key default gen_random_uuid(),user_id uuid not null references auth.users(id) on delete cascade,kind text not null,payload jsonb not null default '{}'::jsonb,read_at timestamptz,created_at timestamptz not null default now());
alter table public.notifications_v2 enable row level security;
create policy "notification owner" on public.notifications_v2 for select using(user_id=auth.uid());
...
declare v_actor uuid:=auth.uid();v_duel public.duels;v_response jsonb;v_end timestamptz;v_challenger_score integer;v_challenged_score integer;v_winner uuid;
begin
