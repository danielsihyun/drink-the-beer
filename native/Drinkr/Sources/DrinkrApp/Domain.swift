...
public struct Cursor: Codable, Equatable, Sendable { public let takenAt: Date; public let id: UUID; public var encoded: String { Data(try! JSONEncoder.drinkr.encode(self)).base64EncodedString() }
  public init(takenAt: Date, id: UUID) { self.takenAt = takenAt; self.id = id }
  public init(from decoder: Decoder) throws { if let value = try? decoder.singleValueContainer().decode(String.self) { guard let data = Data(base64Encoded: value, options: .ignoreUnknownCharacters) else { throw DecodingError.dataCorruptedError(in: try decoder.singleValueContainer(), debugDescription: "Invalid feed cursor.") }; struct WireCursor: Decodable { let takenAt: Date; let id: UUID }; let cursor = try JSONDecoder.drinkr.decode(WireCursor.self, from: data); takenAt = cursor.takenAt; id = cursor.id; return }; let c = try decoder.container(keyedBy: CodingKeys.self); takenAt = try c.decode(Date.self, forKey: .takenAt); id = try c.decode(UUID.self, forKey: .id) }
}
public struct FeedPost: Decodable, Identifiable, Sendable { public let id: UUID; public let authorName: String; public let drinkName: String; public let caption: String?; public let takenAt: Date; public let mediaAssetID: UUID?; public var cheersCount: Int; public var viewerCheered: Bool; public var pending: Bool; enum CodingKeys: String, CodingKey { case id, authorName, drinkName, caption, takenAt, cheersCount, viewerCheered, pending; case mediaAssetID = "mediaAssetId" }
  public init(from decoder: Decoder) throws { let c = try decoder.container(keyedBy: CodingKeys.self); id = try c.decode(UUID.self, forKey: .id); authorName = (try? c.decode(String.self, forKey: .authorName)) ?? "Unknown"; drinkName = (try? c.decode(String.self, forKey: .drinkName)) ?? "Drink"; caption = try? c.decodeIfPresent(String.self, forKey: .caption); takenAt = (try? c.decode(Date.self, forKey: .takenAt)) ?? .distantPast; mediaAssetID = try? c.decodeIfPresent(UUID.self, forKey: .mediaAssetID); cheersCount = (try? c.decode(Int.self, forKey: .cheersCount)) ?? Int((try? c.decode(String.self, forKey: .cheersCount)) ?? "") ?? 0; viewerCheered = (try? c.decode(Bool.self, forKey: .viewerCheered)) ?? ((try? c.decode(String.self, forKey: .viewerCheered)) == "true"); pending = (try? c.decode(Bool.self, forKey: .pending)) ?? false }
}
...
extension JSONDecoder { static var drinkr: JSONDecoder { let d=JSONDecoder(); d.dateDecodingStrategy = .custom { decoder in let container = try decoder.singleValueContainer(); if let seconds = try? container.decode(Double.self) { return Date(timeIntervalSince1970: seconds) }; let value = try container.decode(String.self); let fractional = ISO8601DateFormatter(); fractional.formatOptions = [.withInternetDateTime, .withFractionalSeconds]; if let date = fractional.date(from: value) { return date }; let standard = ISO8601DateFormatter(); standard.formatOptions = [.withInternetDateTime]; if let date = standard.date(from: value) { return date }; throw DecodingError.dataCorruptedError(in: container, debugDescription: "Expected an ISO-8601 timestamp.") }; return d } }
